...
?>
	...
	<br>
	<?php
	$re = mysql_query('SELECT username,parent,rating FROM rating');
	if (mysql_num_rows($re) > 0) {
     while($row = mysql_fetch_array($re)) {
         if($_SESSION['username']==$row["username"] and $id==$row["parent"]) {
		$flag=1;
		break;
		}
	else {
	...
	if($flag==1){
		$rating=$row["rating"];
		echo '<br>';
		echo "You have already rated $rating for this category!";
?>
		<form method="post" action="add_ratings.php">
	<a href="<?php echo $_SERVER['PHP_SELF']; ?>?parent=<?php echo $id; ?>" class="button">Update your rating?</a>
	<input type="hidden" name="username" value="<?php echo $_SESSION['username'];?>" />
	<input type="hidden" name="parent" value="<?php echo $id;?>" />
	<input type="hidden" name="redirect" value="<?php echo $_SERVER['PHP_SELF']; ?>?parent=<?php echo $id; ?>" />
	<select name='rate'>
...
<?php
	...
	<form method="post" action="add_ratings.php">
	<br>
	<a href="<?php echo $_SERVER['PHP_SELF']; ?>?parent=<?php echo $id; ?>" class="button">Rate this Category!</a>
	<input type="hidden" name="username" value="<?php echo $_SESSION['username'];?>" />
<input type="hidden" name="parent" value="<?php echo $id;?>" />
	<input type="hidden" name="redirect" value="<?php echo $_SERVER['PHP_SELF']; ?>?parent=<?php echo $id; ?>" />
	<select name='rate'>
